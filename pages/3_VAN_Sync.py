...
OVERVIEW_BOOKMARKLET_JS = r"""javascript:void(function(){var t=document.querySelectorAll('table.rgMasterTable tbody tr, table[id*="Grid"] tbody tr');if(!t.length){alert('No table found. Open SmartVAN Activity Report first.');return}var d=[];t.forEach(function(r){var c=r.querySelectorAll('td');if(c.length>=11){var name=c[3]?c[3].innerText.trim():'';if(name&&name!==''){d.push({canvasser:name,listName:c[0]?c[0].innerText.trim():'',attempts:parseInt(c[4]?c[4].innerText.trim():0)||0,doors:parseInt(c[5]?c[5].innerText.trim():0)||0,canvassed:parseInt(c[6]?c[6].innerText.trim():0)||0,notHome:parseInt(c[8]?c[8].innerText.trim():0)||0,moved:parseInt(c[9]?c[9].innerText.trim():0)||0,refused:parseInt(c[10]?c[10].innerText.trim():0)||0,status:c[11]?c[11].innerText.trim():'P'})}}});if(!d.length){alert('No data rows found.');return}var prev=localStorage.getItem('van_scrape');if(prev){try{var old=JSON.parse(prev);d=old.concat(d)}catch(e){}}localStorage.setItem('van_scrape',JSON.stringify(d));var more=confirm('Scraped '+d.length+' rows total (this page: '+t.length+').\n\nMore pages? Click Cancel to accumulate, OK to copy & finish.');if(more){var txt=JSON.stringify(d);navigator.clipboard.writeText(txt).then(function(){localStorage.removeItem('van_scrape');alert('Copied '+d.length+' canvassers to clipboard.\nPaste into cantrack.pro/VAN_Sync')}).catch(function(){localStorage.removeItem('van_scrape');prompt('Copy this JSON:',txt)})}})()"""
